...
long_dis <- melt(op_visit[,.(PatientID, ICD10_1, ICD10_2, ICD10_3)], id.vars = c("PatientID"))

long_dis <- DiseaseMap[long_dis, on = .(ICD10 = value)][patient, on = .(PatientID = PatientID)][,.(Condition, PatientID, Gender)]
...
# Get totals
final_dis[,.(female_total = sum(female),
             male_total = sum(male),
...
This isn't right. The denominator needs to be the total number of male/female patients. Will need to redo.
